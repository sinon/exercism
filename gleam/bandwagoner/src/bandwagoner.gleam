...
pub fn root_for_team(team: Team) -> Bool {
  let is_fav_coach = team.coach.name == "Gregg Popovich"
  let former_player = team.coach.former_player
  let is_bulls = team.name == "Chicago Bulls"
  let over_60 = team.stats.wins >= 60
  let underdog = team.stats.losses > team.stats.wins
  list.any([is_fav_coach, former_player, is_bulls, over_60, underdog], fn(x) {
    x == True
  })
}
