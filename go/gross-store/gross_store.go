...
func RemoveItem(bill, units map[string]int, item, unit string) bool {
	billAmount, hasItemOnBill := bill[item]
	amountToRemove, hasUnit := units[unit]
	if !hasItemOnBill || !hasUnit {
		return false
	}
	if billAmount-amountToRemove < 0 {
		return false
	}
	if billAmount-amountToRemove == 0 {
		delete(bill, item)
	} else {
		bill[item] = billAmount - amountToRemove
	}
	return true
}
